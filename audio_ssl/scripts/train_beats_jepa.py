...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Continued SSL on MIMII with a pretrained BEATs encoder (data2vec-style JEPA).")
    parser.add_argument("--config", default="audio_ssl/configs/beats_jepa.yaml")
    parser.add_argument("--base-directory")
    parser.add_argument("--run-dir")
    parser.add_argument("--max-epochs", type=int)
    parser.add_argument("--init-only", action="store_true",
                        help="Save the UNTRAINED (pretrained-BEATs) module as last.ckpt and exit — "
                             "the frozen-probe baseline for the standard embedding eval.")
...
def save_bare_checkpoint(module: pl.LightningModule, path: Path) -> None:
    """Lightning-loadable checkpoint without a Trainer (for --init-only)."""
...
def main() -> None:
    ...
    beats_ckpt = model_cfg["beats_checkpoint"]
    beats_cfg = BEATsEncoder.load_pretrained_cfg(beats_ckpt)  # stored in hparams -> eval needs no .pt
    module = LitBEATsJEPA(
        beats_cfg=beats_cfg, beats_checkpoint=beats_ckpt,
        target_frames=int(feature_cfg["target_frames"]),
        finetune_last_n=int(model_cfg.get("finetune_last_n", 2)),
        head_hidden=int(model_cfg.get("head_hidden", 768)),
        num_blocks=int(model_cfg.get("num_blocks", 4)),
        mask_scale=tuple(model_cfg.get("mask_scale", [0.15, 0.3])),
        mask_aspect=tuple(model_cfg.get("mask_aspect", [0.5, 2.0])),
        ema_start=float(model_cfg.get("ema_start", 0.999)),
        ema_end=float(model_cfg.get("ema_end", 1.0)),
        lr=float(fit_cfg["lr"]), weight_decay=float(fit_cfg.get("weight_decay", 0.01)),
    ...
    if args.init_only:
        if global_rank == 0:
            save_bare_checkpoint(module, checkpoint_dir / "last.ckpt")
            print(f"[init-only] frozen pretrained BEATs saved -> {checkpoint_dir / 'last.ckpt'}", flush=True)
        return
    ...
    loggers = build_loggers(
        ...
        extra_params={"run": output_dir.name, "arch": "beats", "n_train_files": len(train_files),
                      "finetune_last_n": int(model_cfg.get("finetune_last_n", 2)),
    ...
    checkpoint = ModelCheckpoint(dirpath=checkpoint_dir, filename="{epoch:03d}", save_last=True,
                                 save_top_k=-1, every_n_epochs=int(ckpt_cfg.get("every_n_epochs", 5)),
